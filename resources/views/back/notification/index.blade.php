@if (App\Models\Notification::count() > 0)
    <h6 class="dropdown-header">
        {{ __('Notifications') }}
        <a class="text-dark float-right" id="clear-notf" data-href="{{ route('back.notifications.clear') }}"
            href="javascript:;">
    ...
    @endforeach
    @if (App\Models\Notification::count() > 0)
        <a class="dropdown-header mt-1 d-block text-center" href="{{ route('back.view.notification') }}">
            {{ __('View All') }} </a>
    @endif
@else
    <h6 class="dropdown-header">
