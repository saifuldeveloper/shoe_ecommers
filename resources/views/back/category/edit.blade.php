							...
							<form class="admin-form" action="{{ route('back.category.update',$category->id) }}"
								...
								<div class="form-group">
									...
									<select name="is_in_menu" id="is_in_menu" class="form-control">
										...
										<option value="1" {{ $category->is_in_menu == 1 ? 'selected' : '' }}>{{ __('Yes') }}</option>
									</select>
								</div>
								...
								<div class="form-group">
									...
									<input type="number" name="menu_serial" class="form-control" id="menu_serial"
										placeholder="{{ __('Enter Menu Serial Number') }}" value="{{ $category->menu_serial }}">
								</div>

								<div class="form-group">
									<label for="is_featured">{{ __('Show as Featured?') }}</label>
									<select name="is_featured" id="is_featured" class="form-control">
										<option value="0" {{ $category->is_featured == 0 ? 'selected' : '' }}>{{ __('No') }}</option>
										<option value="1" {{ $category->is_featured == 1 ? 'selected' : '' }}>{{ __('Yes') }}</option>
									</select>
								</div>
								...
								<div class="form-group">
									...
									<input type="number" name="featured_serial" class="form-control" id="featured_serial"
										placeholder="{{ __('Enter Featured Serial Number') }}" value="{{ $category->featured_serial }}">
								</div>
