...
class ItemRepository
{
    ...
    public function restore($id)
    {
        ...
        if ($item) {
            $item->restore();
            return ['message' => __('Item Restored Successfully.'), 'status' => 1];
        } else {
            return ['message' => __('Item not found.'), 'status' => 0];
        }
    }
    ...
    public function forceDelete($id)
    {
        ...
        if ($item) {
            ...
            ImageHelper::handleDeletedImage($item, 'thumbnail', 'items');
            $item->forceDelete();
            return ['message' => __('Item Permanently Deleted Successfully.'), 'status' => 1];
        } else {
            return ['message' => __('Item not found.'), 'status' => 0];
        }
    }
